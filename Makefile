...
ifeq ("$(MAKECMDGOALS)", "fifo")
OBJS += $K/fifo.o
endif
ifeq ("$(MAKECMDGOALS)", "lru")
OBJS += $K/lru.o
endif

ifeq ("$(MAKECMDGOALS)", "fifo-gdb")
OBJS += $K/fifo.o
endif
ifeq ("$(MAKECMDGOALS)", "lru-gdb")
OBJS += $K/lru.o
endif
...
ifeq ("$(MAKECMDGOALS)", "lru")
CFLAGS += -DPG_REPLACEMENT_USE_LRU=1
endif
ifeq ("$(MAKECMDGOALS)", "fifo")
CFLAGS += -DPG_REPLACEMENT_USE_FIFO=1
endif
...
fifo-gdb: $K/kernel .gdbinit fs.img
	@echo "*** Now run 'gdb' in another window." 1>&2
	$(QEMU) $(QEMUOPTS) -S $(QEMUGDB)

lru-gdb: $K/kernel .gdbinit fs.img
	@echo "*** Now run 'gdb' in another window." 1>&2
	$(QEMU) $(QEMUOPTS) -S $(QEMUGDB)
...
grade:
	...
	@python3 run_mp2.py 2>&1 | tee result.csv
	@$(MAKE) clean > /dev/null
...
zip:
	...
	mkdir $(STUDENT_ID)
	mkdir $(STUDENT_ID)/xv6
	cp -rt $(STUDENT_ID)/xv6 kernel user mkfs conf Makefile
